...
func (s *TransactionGRPCServer) FetchEventsToArchive(ctx context.Context, req *pb.FetchEventsToArchiveRequest) (*pb.FetchEventsToArchiveResponse, error) {
	logger.Log.InfoContext(ctx, "[gRPC] FetchEventsToArchive called", slog.Int64("min_age_seconds", req.MinAgeSeconds), slog.Int("limit", int(req.Limit)))

	minAge := time.Duration(req.MinAgeSeconds) * time.Second
	events, err := s.repo.FetchEventsToArchive(ctx, minAge, int(req.Limit))
	if err != nil {
		logger.Log.ErrorContext(ctx, "[gRPC] FetchEventsToArchive failed", slog.Any("error", err))
	...
	logger.Log.InfoContext(ctx, "[gRPC] FetchEventsToArchive success", slog.Int("count", len(events)))
	return &pb.FetchEventsToArchiveResponse{Events: pbEvents}, nil
}

func (s *TransactionGRPCServer) DeleteArchivedEvents(ctx context.Context, req *pb.DeleteArchivedEventsRequest) (*pb.DeleteArchivedEventsResponse, error) {
	logger.Log.InfoContext(ctx, "[gRPC] DeleteArchivedEvents called", slog.Int("count", len(req.Ids)))
